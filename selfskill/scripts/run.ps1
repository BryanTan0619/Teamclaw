...
switch ($Command) {
    ...
    "start" {
        ...
        $agentPort = [int](Get-EnvValue -Key "PORT_AGENT" -DefaultValue "51200")
        ...
        if (Wait-HttpEndpoint -Url "http://127.0.0.1:$agentPort/v1/models") {
            ...
            Write-Host "Service may still be starting. Check the logs if needed."
        }
    ...
    "status" {
        ...
        $ports = @(
            [int](Get-EnvValue -Key "PORT_AGENT" -DefaultValue "51200"),
            [int](Get-EnvValue -Key "PORT_SCHEDULER" -DefaultValue "51201"),
            [int](Get-EnvValue -Key "PORT_OASIS" -DefaultValue "51202"),
            [int](Get-EnvValue -Key "PORT_FRONTEND" -DefaultValue "51209")
        )
        ...
        foreach ($port in $ports) {
            if (Test-LocalPort -Port $port) {
                ...
                Write-Host "  Port $port is not listening yet"
            }
        }
    ...
    "check-openclaw" {
        ...
        if (-not $node -or -not $npm) {
            ...
            Write-Host "On native Windows, install Node.js 22+ first, or use the WSL flow in docs/windows.md."
            exit 1
        }
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "OpenClaw installation failed. Please check npm and your network connection."
        }
